...
@Component({
  selector: 'mini-calendar-picker',
  templateUrl: './mini-calendar-picker.component.html',
  styleUrls: ['./mini-calendar-picker.component.scss']
})
export class MiniCalendarPickerComponent implements OnInit, AfterViewInit {
  @Input() cssClasses = '';
  ...
  @Input() today;
  @Output() todayChange = new EventEmitter<any>();
  weekDays;
  ...
  ngAfterViewInit(): void {
    ...
    if (this.selected == undefined){
      ...
      this.selected = this.month.clone();
    }

    this.today = this.month.clone();
    this.todayChange.emit(this.today);
    ...
    this.bodyDivs.forEach(day => {
      day.addEventListener('click', e => {
        ...
        if (e.target.classList.contains('mini-cal-day__month--next')) {
          this.selected = moment(`${this.month.add(1, 'month').format('YYYY-MM')}-${date}`)
        } else if (e.target.classList.contains('mini-cal-day__month--previous')) {
  ...
  draw () {
    ...
    if (this.calendarDays.first > 1) {
      for (let day = this.calendarDays.first; day <= this.monthDays.lastPrevious; index ++) {
        ...
        //this.bodyDivs[index].classList.add('mini-cal-day__month--previous')
      } 
    }
    ...
    for (let day = 1; index <= 6*7 - 1; index ++) {
      ...
      if (!isNextMonth) {
        ...
        if (day === this.today.date() && this.today.isSame(this.month, 'day')) {
          // this.bodyDivs[index].classList.add('mini-cal-day__day--today') 
          classe += ' mini-cal-day__day--today'
        }

        if (day === this.selected.date() && this.selected.isSame(this.month, 'month')) {
          // this.bodyDivs[index].classList.add('mini-cal-day__day--selected') 
          classe += ' mini-cal-day__day--selected'
          selected = true;
        }

        // this.bodyDivs[index].classList.add('mini-cal-day__month--current')
        
        mes = "atual";
      } else {
        // this.bodyDivs[index].classList.add('mini-cal-day__month--next')
        classe = ' mini-cal-day__month--next'
        mes = "proximo"
  ...
  cleanCssClasses (selected, index) {
    this.bodyDivs[index].classList.contains('mini-cal-day__month--next') && 
      this.bodyDivs[index].classList.remove('mini-cal-day__month--next')
    this.bodyDivs[index].classList.contains('mini-cal-day__month--previous') && 
      this.bodyDivs[index].classList.remove('mini-cal-day__month--previous')
    this.bodyDivs[index].classList.contains('mini-cal-day__month--current') &&
      this.bodyDivs[index].classList.remove('mini-cal-day__month--current')
    this.bodyDivs[index].classList.contains('mini-cal-day__day--today') && 
      this.bodyDivs[index].classList.remove('mini-cal-day__day--today')
    if (selected) {
      this.bodyDivs[index].classList.contains('mini-cal-day__day--selected') && 
        this.bodyDivs[index].classList.remove('mini-cal-day__day--selected') 
    }
  }
