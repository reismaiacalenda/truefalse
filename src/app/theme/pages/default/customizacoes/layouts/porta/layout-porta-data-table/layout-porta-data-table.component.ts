...
@Component({
  selector: 'layout-porta-data-table',
  templateUrl: './layout-porta-data-table.html'
})
export class LayoutPortaDataTableComponent implements OnInit {
  ...
  carregarTable() {
    ...
    this.http.get(`${this.apiUrl}.json`)
      .subscribe(
      dados => this.rows = this.dados = (<any>dados).layouts
      )
  ...
  openFormModal(rowId?: string) {
    ...
    const modalRef = this.modalNgb.open(LayoutPortaFormComponent, ngbModalOptions);
