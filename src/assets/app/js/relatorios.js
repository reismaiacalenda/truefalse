...
var Dashboard = function() {
  ...
  //== Trends Stats.
  //** Based on Chartjs plugin - http://www.chartjs.org/
  var reservasPorMes = function() {
      if ($('#reservasPorMes').length == 0) {
          return;
      }

      var ctx = document.getElementById("reservasPorMes").getContext("2d"); 

      var gradient = ctx.createLinearGradient(0, 0, 0, 240);
      gradient.addColorStop(0, Chart.helpers.color('#5867dd').alpha(0.7).rgbString());   
      gradient.addColorStop(1, Chart.helpers.color('#abb0d8').alpha(0).rgbString());
  ...
  //** Based on Morris plugin - http://morrisjs.github.io/morris.js/
  var reservasPorDuracao = function() {
      if ($('#reservasPorDuracaoLabel').length == 0) {
          return;
      }
    //   var ovo = 
    //   
    Morris.Donut({
          element: 'reservasPorDuracaoLabel',
          data: reservasPorDuracaoData,
          labelColor: '#a7a7c2',
          colors: [
            mUtil.getColor('danger'),
  ...
  var bandwidthChart2 = function() {
      ...
      var gradient = ctx.createLinearGradient(0, 0, 0, 240);
      gradient.addColorStop(0, Chart.helpers.color('#aca9d3').alpha(1).rgbString());   
      gradient.addColorStop(1, Chart.helpers.color('#aca9d3').alpha(0.3).rgbString());

      var config = {
          type: 'line',
          data: {
              labels: frequenciaHorarios,
              datasets: [{
                  label: "Reuniões",
                  backgroundColor : gradient,
                  borderColor : mUtil.getColor('brand'),
  ...
  var reservasPorFuncionario = function(){

    var chartData = {
        labels: reservasPorFuncionarioLabel,
        datasets: [{
            label: 'Reservas',
            backgroundColor: mUtil.getColor('brand'),
            data: reservasPorFuncionarioData
        }]
    };

    var chartContainer = $('#reservasPorFuncionario');
